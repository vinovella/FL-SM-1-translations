# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:15
translate italian sm1cs_dc_renovation_b8cfd8d1:

    # "*Knock, knock*"
    "*Toc, toc*"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:18
translate italian sm1cs_dc_renovation_be7d532d:

    # mct "That must be Debbie!"
    mct "Deve essere Debbie!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:22
translate italian sm1cs_dc_renovation_7daa0cc7:

    # mc "Coming!"
    mc "Arrivo!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:26
translate italian sm1cs_dc_renovation_16b05ad9:

    # mc "Debbie?"
    mc "Debbie?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:33
translate italian sm1cs_dc_renovation_3c0c267e:

    # dc "Yes?"
    dc "Sì?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:34
translate italian sm1cs_dc_renovation_85cfe7c6:

    # dc "What, is something wrong?"
    dc "Cosa c'è, qualcosa non va?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:39
translate italian sm1cs_dc_renovation_5c8bfeef:

    # mc "Uhm, no! I'm just realizing I've never seen you out of uniform before."
    mc "Uhm, no! Mi sto solo rendendo conto che non ti avevo mai vista senza uniforme."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:41
translate italian sm1cs_dc_renovation_9f993911:

    # mc "Uhm, no! I just forget what you look like without your cop hat."
    mc "Uhm, no! Avevo dimenticato come sei senza il cappello da poliziotta."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:47
translate italian sm1cs_dc_renovation_8ed9c4f4:

    # dc "What, did you think the only outfit I had was my work uniform?"
    dc "Cosa, pensavi che l'unico vestito che avevo fosse la mia uniforme da lavoro?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:50
translate italian sm1cs_dc_renovation_7136c946:

    # mc "I mean..."
    mc "Beh, voglio dire..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:53
translate italian sm1cs_dc_renovation_ce2cf84c:

    # dc "Oh stop it, you!"
    dc "Oh smettila!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:58
translate italian sm1cs_dc_renovation_29ead0f5:

    # dc "Wow, this is a great place you've got here, [mcname]."
    dc "Wow, hai un posto fantastico qui, [mcname]."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:61
translate italian sm1cs_dc_renovation_da374159:

    # mc "Thanks, I have a killer real estate agent who found it for me!"
    mc "Grazie, ho un agente immobiliare fantastico che l'ha trovato per me!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:64
translate italian sm1cs_dc_renovation_b630e691:

    # dc "I'll have to get their number if I ever decide to move. The guy who rented me my apartment was..."
    dc "Dovrò farmi dare il suo numero se mai deciderò di trasferirmi. Il tipo che mi ha affittato l'appartamento era..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:67
translate italian sm1cs_dc_renovation_c521a2d5:

    # dc "Well, let's just say that if I had any other option for a place to live, I would've taken it."
    dc "Beh, diciamo solo che se avessi avuto qualsiasi altra opzione per un posto dove vivere, l'avrei presa."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:68
translate italian sm1cs_dc_renovation_825fb96f:

    # dc "But enough about my troubles! I'm here to help you!"
    dc "Ma basta con i miei problemi! Sono qui per aiutarti!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:69
translate italian sm1cs_dc_renovation_d62d9525:

    # dc "So what still needs to be done?"
    dc "Allora, cosa c'è ancora da fare?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:72
translate italian sm1cs_dc_renovation_99a6d8ec:

    # mc "Let's see... I think some of the uhm... 2 by 4s in the walls need reinforcing. I know we're going to hang lots of photos."
    mc "Vediamo... Penso che alcuni dei uhm... montanti nelle pareti debbano essere rinforzati. So che appenderemo un sacco di foto."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:73
translate italian sm1cs_dc_renovation_3d449cfe:

    # mc "And there's been this weird problem with the lights I can't fix today."
    mc "E c'è questo strano problema con le luci che non riesco a risolvere."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:76
translate italian sm1cs_dc_renovation_e30086de:

    # dc "Well why don't I get a hammer and I can start with the walls?"
    dc "Beh, perché non prendo un martello e comincio con le pareti?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:79
translate italian sm1cs_dc_renovation_4954dfd6:

    # mc "That sounds great!"
    mc "Ottima idea!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:83
translate italian sm1cs_dc_renovation_d24bc72b:

    # mc "Let's see..."
    mc "Vediamo..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:87
translate italian sm1cs_dc_renovation_7a7f7d2b:

    # mc "Here you go!"
    mc "Here you go!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:90
translate italian sm1cs_dc_renovation_9bdd3f10:

    # dc "Thanks, [mcname]!"
    dc "Grazie, [mcname]!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:117
translate italian sm1cs_dc_renovation_ac21ba5c:

    # dc "Wow, you weren't kidding about how much work this place needed!"
    dc "Wow, non scherzavi su quanto lavoro servisse a questo posto!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:120
translate italian sm1cs_dc_renovation_69c25b65:

    # mc "Yeah..."
    mc "Già..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:123
translate italian sm1cs_dc_renovation_7a6572e8:

    # dc "How's this coming along?"
    dc "Come sta andando?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:126
translate italian sm1cs_dc_renovation_70a444bd:

    # mc "I don't know."
    mc "Non lo so."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:129
translate italian sm1cs_dc_renovation_fe38d4b5:

    # mc "I've tried everything I could think of... I still can't get the lights to turn on."
    mc "Ho provato tutto quello a cui riuscivo a pensare... Non riesco ancora a far accendere le luci."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:132
translate italian sm1cs_dc_renovation_35297986:

    # dc "You sure there's a bulb screwed into the socket?"
    dc "Sei sicuro che ci sia una lampadina avvitata nella presa?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:136
translate italian sm1cs_dc_renovation_7bc8da4c:

    # mc "Yeah. New bulbs, new wires, new everything. I don't know what I'm doing wrong."
    mc "Sì. Lampadine nuove, fili nuovi, tutto nuovo. Non so cosa sto sbagliando."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:139
translate italian sm1cs_dc_renovation_0cad315b:

    # dc "Do you mind if I take a look?"
    dc "Ti dispiace se do un'occhiata?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:142
translate italian sm1cs_dc_renovation_faed8d95:

    # mc "Please."
    mc "Ti prego."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:146
translate italian sm1cs_dc_renovation_217a8187:

    # dc "Hmmm."
    dc "Hmmm."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:149
translate italian sm1cs_dc_renovation_4a07df0e:

    # mc "Do you see something?"
    mc "Vedi qualcosa?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:152
translate italian sm1cs_dc_renovation_63422d5f:

    # dc "Maybe...{w} have you got a screwdriver?"
    dc "Forse...{w} hai un cacciavite?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:155
translate italian sm1cs_dc_renovation_8390bb7b:

    # mc "Somewhere around here, yeah."
    mc "Da qualche parte qui intorno, sì."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:162
translate italian sm1cs_dc_renovation_7a7f7d2b_1:

    # mc "Here you go!"
    mc "Ecco qui!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:172
translate italian sm1cs_dc_renovation_68cd5756:

    # dc "I'll just give that a little jiggle... and tighten that... and..."
    dc "Gli darò solo una piccola scossa... e stringerò quello... e..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:178
translate italian sm1cs_dc_renovation_7ee3ace8:

    # mc "Oh my God! You did it! How!?"
    mc "Oh mio Dio! Ce l'hai fatta! Come!?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:181
translate italian sm1cs_dc_renovation_41f5cc63:

    # dc "Oh, it was a silly problem."
    dc "Oh, era un problema stupido."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:185
translate italian sm1cs_dc_renovation_f7c21acd:

    # dc "The way this switch is wired has a ground wire that was all tangled up."
    dc "Il modo in cui è cablato questo interruttore ha un filo di terra che era tutto aggrovigliato."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:186
translate italian sm1cs_dc_renovation_51e4b1d1:

    # dc "And one of your connections was loose. You had it like 90 percent of the way there."
    dc "E una delle connessioni era allentata. Eri al 90 percento della soluzione."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:189
translate italian sm1cs_dc_renovation_a1c774a3:

    # mc "Really? That was it?"
    mc "Davvero? Era questo?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:193
translate italian sm1cs_dc_renovation_21622144:

    # dc "Yep, that was it. I think you just spent too long staring into that little hole."
    dc "Sì, era questo. Penso che tu abbia passato troppo tempo a fissare quel piccolo buco."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:194
translate italian sm1cs_dc_renovation_7497e470:

    # dc "Tunnel vision gets even the-"
    dc "La visione a tunnel colpisce anche i-"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:198
translate italian sm1cs_dc_renovation_43182693:

    # dc "Oh my God..."
    dc "Oh mio Dio..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:201
translate italian sm1cs_dc_renovation_a47a9c1f:

    # mc "What? Are you okay?"
    mc "Cosa? Stai bene?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:204
translate italian sm1cs_dc_renovation_7a6af41c:

    # dc "I, erm..."
    dc "Io, ehm..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:206
translate italian sm1cs_dc_renovation_e628426c:

    # mc "Debbie, you can tell me."
    mc "Debbie, puoi dirmelo."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:207
translate italian sm1cs_dc_renovation_72d62a83:

    # dc "I just heard what I said and... it sounds like such an innuendo."
    dc "Ho appena sentito quello che ho detto e... sembra proprio un'allusione."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:210
translate italian sm1cs_dc_renovation_f0805b8e:

    # mc "That wasn't what I was thinking, but now that you've said it..."
    mc "Non era quello a cui stavo pensando, ma ora che l'hai detto..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:211
translate italian sm1cs_dc_renovation_abf0c108:

    # mc "I think I get tunnel vision every time I stare into a little hole!"
    mc "Penso di avere la visione a tunnel ogni volta che fisso un piccolo buco!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:214
translate italian sm1cs_dc_renovation_07ab98c5:

    # dc "Ha, ha. Very funny, [mcname]."
    dc "Ah, ah. Molto divertente, [mcname]."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:220
translate italian sm1cs_dc_renovation_e7efd5e7:

    # mc "Oh shit! I haven't offered you water or anything! You must have world up a thirst."
    mc "Oh merda! Non ti ho offerto dell'acqua o altro! Devi aver fatto venire sete."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:223
translate italian sm1cs_dc_renovation_892af79d:

    # dc "Water actually sounds really nice right now."
    dc "Dell'acqua suona davvero bene in questo momento."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:228
translate italian sm1cs_dc_renovation_57e0c03b:

    # dc "Man... your apartment is so big!"
    dc "Cavolo... il tuo appartamento è così grande!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:231
translate italian sm1cs_dc_renovation_b3189473:

    # mc "Well, part of that is also because we plan to film here."
    mc "Beh, in parte è anche perché abbiamo intenzione di girare qui."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:234
translate italian sm1cs_dc_renovation_78e20b2f:

    # dc "I remember you saying that. I just thought it might be, I don't know... smaller. And that there wouldn't be so many bedrooms."
    dc "Ricordo che l'hai detto. Pensavo solo che sarebbe stato, non so... più piccolo. E che non ci sarebbero state così tante camere da letto."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:239
translate italian sm1cs_dc_renovation_dade2980:

    # mc "Yep! I figure if I can live at where I work, I can save some money on rent."
    mc "Esatto! Penso che se posso vivere dove lavoro, posso risparmiare un po' di soldi sull'affitto."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:242
translate italian sm1cs_dc_renovation_496c0cfe:

    # dc "That's pretty smart."
    dc "È piuttosto intelligente."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:245
translate italian sm1cs_dc_renovation_92b4e2df:

    # mc "You have to do whatever you have to do to make it as a small business!"
    mc "Devi fare tutto quello che devi fare per farcela come piccola impresa!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:249
translate italian sm1cs_dc_renovation_67f88316:

    # dc "Very true!"
    dc "Verissimo!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:256
translate italian sm1cs_dc_renovation_e1fba0dc:

    # dc "Oh, that really hit the spot. Thank you, [mcname]."
    dc "Oh, proprio quello che ci voleva. Grazie, [mcname]."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:259
translate italian sm1cs_dc_renovation_85d475fe:

    # mc "Of course!"
    mc "Di niente!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:263
translate italian sm1cs_dc_renovation_59c08bc6:

    # mc "So I'm curious... when did you get so good at doing this kind of work?"
    mc "Allora sono curioso... quando sei diventata così brava a fare questo tipo di lavoro?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:266
translate italian sm1cs_dc_renovation_dee04957:

    # dc "Why? Do I not look like someone who would be handy?"
    dc "Perché? Non sembro una che sarebbe pratica?"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:269
translate italian sm1cs_dc_renovation_f3592667:

    # mc "No, I just thought you were a cop and not a carpenter."
    mc "No, pensavo solo che fossi una poliziotta e non una falegname."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:272
translate italian sm1cs_dc_renovation_3468a884:

    # dc "Fair! Well..."
    dc "Giusto! Beh..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:279
translate italian sm1cs_dc_renovation_f2c26dbb:

    # dc "When I lived out on the coast, the town I was in was super small."
    dc "Quando vivevo sulla costa, la città in cui mi trovavo era super piccola."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:280
translate italian sm1cs_dc_renovation_ee41fab3:

    # dc "If something broke, it would take forever to get someone to come out and fix it."
    dc "Se qualcosa si rompeva, ci voleva un'eternità per far venire qualcuno a ripararlo."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:281
translate italian sm1cs_dc_renovation_37129ff1:

    # dc "So I started learning how to fix things around the house, because they needed fixing."
    dc "Così ho iniziato a imparare come riparare le cose in casa, perché dovevano essere riparate."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:284
translate italian sm1cs_dc_renovation_e1b24751:

    # dc "I'll never forget... the first thing that I had to fix was a busted water pipe!"
    dc "Non dimenticherò mai... la prima cosa che ho dovuto riparare è stato un tubo dell'acqua rotto!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:287
translate italian sm1cs_dc_renovation_7168d4f3:

    # mc "What!"
    mc "Cosa!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:290
translate italian sm1cs_dc_renovation_92fab953:

    # dc "Yep. It was my shower line... I had just got home from a patrol on the beach, I was covered in sand and sweat."
    dc "Sì. Era il tubo della doccia... Ero appena tornata a casa da una pattuglia sulla spiaggia, ero coperta di sabbia e sudore."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:293
translate italian sm1cs_dc_renovation_5051d64b:

    # dc "I walked in the door, went to the bathroom, and there was an inch of water on the ground!"
    dc "Sono entrata dalla porta, sono andata in bagno, e c'erano alcuni centimetri d'acqua per terra!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:294
translate italian sm1cs_dc_renovation_5139034b:

    # dc "I tried calling the plumber, but he said it was going to be 3 days before he could get to me."
    dc "Ho provato a chiamare l'idraulico, ma ha detto che ci sarebbero voluti 3 giorni prima che potesse venire da me."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:295
translate italian sm1cs_dc_renovation_9d1f47af:

    # dc "And I needed a shower as soon as possible. I had sand in all of the worst places."
    dc "E avevo bisogno di una doccia il prima possibile. Avevo sabbia in tutti i posti peggiori."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:302
translate italian sm1cs_dc_renovation_8210f114:

    # dc "So I started looking things up on the internet, grabbed some tools I had around the house, and started trying to fix it myself."
    dc "Così ho iniziato a cercare cose su internet, ho preso alcuni attrezzi che avevo in casa, e ho iniziato a provare a ripararlo da sola."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:303
translate italian sm1cs_dc_renovation_701e7eaa:

    # dc "It may have taken 6 hours, but I did it!"
    dc "Potranno esserci volute 6 ore, ma ce l'ho fatta!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:306
translate italian sm1cs_dc_renovation_2c9a23fd:

    # mc "Wow..."
    mc "Wow..."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:309
translate italian sm1cs_dc_renovation_84e7b2ad:

    # dc "But since then, I've tried to fix things around my place instead of calling someone."
    dc "Ma da allora, ho provato a riparare le cose a casa mia invece di chiamare qualcuno."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:312
translate italian sm1cs_dc_renovation_bb2066a4:

    # mc "That's pretty awesome, Debbie."
    mc "È davvero fantastico, Debbie."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:315
translate italian sm1cs_dc_renovation_6212163c:

    # dc "Oh, it's nothing. I mean - look at you!"
    dc "Oh, non è niente. Voglio dire - guarda te!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:318
translate italian sm1cs_dc_renovation_d16630e0:

    # dc "You're renovating a whole studio! What I've done is nothing compared to that!"
    dc "Stai ristrutturando un intero studio! Quello che ho fatto io non è niente in confronto!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:321
translate italian sm1cs_dc_renovation_61d1129a:

    # mc "Oh, I don't know about all that."
    mc "Oh, non so se sia proprio così."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:322
translate italian sm1cs_dc_renovation_65011277:

    # mc "We've been here for a few hours, and you got so much done, {i}and{/i} you solved my problem to!"
    mc "Siamo qui da qualche ora, e hai fatto così tanto, {i}e{/i} hai risolto anche il mio problema!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:326
translate italian sm1cs_dc_renovation_28c85fe5:

    # dc "Oh, it was nothing!"
    dc "Oh, non era niente!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:329
translate italian sm1cs_dc_renovation_8dd1ed31:

    # dc "But, speaking of showers, I think I'm going to head home and take one. I'm starting to get a little sticky."
    dc "Ma, parlando di docce, penso che andrò a casa a farne una. Sto iniziando a sentirmi un po' appiccicosa."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:332
translate italian sm1cs_dc_renovation_d20c6bce:

    # mc "Oh yeah! That makes sense."
    mc "Oh sì! Ha senso."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:337
translate italian sm1cs_dc_renovation_4b035a7f:

    # mc "Seriously, Debbie. You were such a help today. Thank you!"
    mc "Sul serio, Debbie. Sei stata di grande aiuto oggi. Grazie!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:340
translate italian sm1cs_dc_renovation_2ac2d23f:

    # dc "Oh, don't mention it, [mcname]. Seriously, I was just happy to help."
    dc "Oh, non c'è di che, [mcname]. Sul serio, ero solo felice di aiutare."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:343
translate italian sm1cs_dc_renovation_69f3f989:

    # dc "Besides, you've helped me catch one criminal, and helped me pursue another."
    dc "Inoltre, mi hai aiutato a catturare un criminale, e mi hai aiutato a inseguirne un altro."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:344
translate italian sm1cs_dc_renovation_cc79d547:

    # dc "I owe you one."
    dc "Ti devo un favore."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:347
translate italian sm1cs_dc_renovation_cbab67ce:

    # mc "Consider us even!"
    mc "Consideraci pari!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:350
translate italian sm1cs_dc_renovation_29e8dbd2:

    # dc "Oh, I don't think we're even."
    dc "Oh, non penso che siamo pari."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:351
translate italian sm1cs_dc_renovation_2e4ea0e6:

    # dc "But I think the scales are a little more balanced now."
    dc "Ma penso che la bilancia sia un po' più equilibrata ora."

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:353
translate italian sm1cs_dc_renovation_4b5b2052:

    # mc "Well, Officer Callahan, we'll have to find a way to make us even!"
    mc "Beh, agente Callahan, dovremo trovare un modo per renderci pari!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:354
translate italian sm1cs_dc_renovation_a25cfcd0:

    # dc "Mmmmm, I guess we will!"
    dc "Mmmmm, immagino di sì!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:359
translate italian sm1cs_dc_renovation_fce227f8:

    # dc "You think on that, and let me know when you come up with something!"
    dc "Pensaci su, e fammi sapere quando ti viene in mente qualcosa!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:362
translate italian sm1cs_dc_renovation_1763033c:

    # mc "I will. Enjoy the rest of your night, Debbie!"
    mc "Lo farò. Goditi il resto della serata, Debbie!"

# game/code/scenes/character_scenes/dc/sm1cs-dc-renovation.rpy:365
translate italian sm1cs_dc_renovation_e5818002:

    # dc "And you, yours, [mcname]!"
    dc "E tu la tua, [mcname]!"

