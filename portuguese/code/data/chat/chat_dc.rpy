translate portuguese strings:

    # game/code/data/chat/chat_dc.rpy:3
    old "Hey, [mcname!i]! I just wanted to say that I'm really happy I finally told you everything, and I am really excited for that future date! 😜"
    new "Hey, [mcname!i]! I just wanted to say that I'm really happy I finally told you everything, and I am really excited for that future date! 😜"

    # game/code/data/chat/chat_dc.rpy:3
    old "Hey Debbie! Im glad we talked too. Im also pretty pumped for this date"
    new "Hey Debbie! Im glad we talked too. Im also pretty pumped for this date"

    # game/code/data/chat/chat_dc.rpy:3
    old "In fact, what are you doing tonight?"
    new "In fact, what are you doing tonight?"

    # game/code/data/chat/chat_dc.rpy:3
    old "I'm actually stuck working the next few night shifts 😞"
    new "I'm actually stuck working the next few night shifts 😞"

    # game/code/data/chat/chat_dc.rpy:3
    old "Booo! Well, maybe I'll run into you in the park"
    new "Booo! Well, maybe I'll run into you in the park"

    # game/code/data/chat/chat_dc.rpy:3
    old "I would love that 😊"
    new "I would love that 😊"

translate portuguese strings:

    # game/code/data/chat/chat_dc.rpy:41
    old "Hey Debbie! 👋"
    new "Hey Debbie! 👋"

    # game/code/data/chat/chat_dc.rpy:41
    old "Hey, [mcname!t]! What can I do for you?"
    new "Hey, [mcname!t]! What can I do for you?"

    # game/code/data/chat/chat_dc.rpy:41
    old "Well I promised you a date and I was wondering if you'd want to go on a date soon? 😊"
    new "Well I promised you a date and I was wondering if you'd want to go on a date soon? 😊"

    # game/code/data/chat/chat_dc.rpy:41
    old "That sounds delightful!"
    new "That sounds delightful!"

    # game/code/data/chat/chat_dc.rpy:41
    old "There's this nice restaurant... wanna go there?"
    new "There's this nice restaurant... wanna go there?"

    # game/code/data/chat/chat_dc.rpy:41
    old "It sounds like a date, [mcname!t] 😉"
    new "It sounds like a date, [mcname!t] 😉"

    # game/code/data/chat/chat_dc.rpy:41
    old "I'll send  you the address! See you in a bit 😏"
    new "I'll send  you the address! See you in a bit 😏"

