translate portuguese strings:

    # game/code/data/chat/chat_kv.rpy:3
    old "Hey Kanya."
    new "Oi Kanya."

    # game/code/data/chat/chat_kv.rpy:3
    old "Hey stud."
    new "Oi garanhão."

    # game/code/data/chat/chat_kv.rpy:3
    old "Hiya [mcname!i]."
    new "Opa [mcname!i]."

    # game/code/data/chat/chat_kv.rpy:3
    old "What's cooking?"
    new "Qual a boa?"

    # game/code/data/chat/chat_kv.rpy:3
    old "Stacy and I are planning to cook up a new film 🎬🎬🎬"
    new "A boa é que eu e a Stacy estamos planejando outro filme🎬🎬🎬"

    # game/code/data/chat/chat_kv.rpy:3
    old "Asking me to be the 🎥 girl again?"
    new "Quer que eu fique de 🎥 de novo?"

    # game/code/data/chat/chat_kv.rpy:3
    old "You know me too well."
    new "Você me conhece bem demais."

    # game/code/data/chat/chat_kv.rpy:3
    old "😝"
    new "😝"

    # game/code/data/chat/chat_kv.rpy:3
    old "What's this one going be about?"
    new "Esse vai ser sobre o quê?"

    # game/code/data/chat/chat_kv.rpy:3
    old "We haven't figured out the details."
    new "Ainda não sabemos os detalhes."

    # game/code/data/chat/chat_kv.rpy:3
    old "But the client wants to focus on anal 🍑"
    new "Mas o cliente quer focar em anal 🍑"

    # game/code/data/chat/chat_kv.rpy:3
    old "🔥🔥🔥"
    new "🔥🔥🔥"

    # game/code/data/chat/chat_kv.rpy:3
    old "Stacy might be in trouble, lol."
    new "Acho que a Stacy se lascou, lol."

    # game/code/data/chat/chat_kv.rpy:3
    old "She'll be fine."
    new "Ela vai ficar de boa."

    # game/code/data/chat/chat_kv.rpy:3
    old "Not our first trip into the mud."
    new "Não é a primeira vez que a gente brinca na lama."

    # game/code/data/chat/chat_kv.rpy:3
    old "Eww. So gross. Haha. Text me when you need me."
    new "Eca. Nojento. Haha. Quando precisar manda mensagem."

    # game/code/data/chat/chat_kv.rpy:3
    old "Sure!"
    new "Beleza!"

translate portuguese strings:

    # game/code/data/chat/chat_kv.rpy:3
    old "What's this one going to be about?"
    new "What's this one going to be about?"

