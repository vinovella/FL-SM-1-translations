translate portuguese strings:

    # game/code/data/quests/quests_theater.rpy:3
    old "Go to the Theater"
    new "Vá ao Teatro"

    # game/code/data/quests/quests_theater.rpy:3
    old "Progress the individual storylines for the Theater girls"
    new "Avance as histórias individuais das garotas do Teatro"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Denise"
    new "Fale com Denise"

    # game/code/data/quests/quests_theater.rpy:3
    old "Read message from Denise"
    new "Leia a mensagem da Denise"

    # game/code/data/quests/quests_theater.rpy:3
    old "Progress the Theater storyline"
    new "Avance a história do Teatro"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Kellie on the stage"
    new "Fale com Kellie no palco"

    # game/code/data/quests/quests_theater.rpy:3
    old "Buy the 'An Actor Prepares' book at the store"
    new "Compre o livro 'Um Ator Se Prepara' na loja"

    # game/code/data/quests/quests_theater.rpy:3
    old "Get your Literature to 5"
    new "Aumente sua literatura para 5"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Kellie"
    new "Fale com Kellie"

    # game/code/data/quests/quests_theater.rpy:3
    old "Text Kellie"
    new "Mande uma mensagem para Kellie"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk with Denise in her office"
    new "Fale com Denise no escritório dela"

    # game/code/data/quests/quests_theater.rpy:3
    old "Have $100 and invite Taisia to make a film"
    new "Tenha $100 e convide Taisia para fazer um filme"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk with Taisia"
    new "Fale com Taisia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Wait for a message from Taisia"
    new "Aguarde uma mensagem da Taisia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Answer Taisia"
    new "Responda a Taisia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk with Stacy about Taisia"
    new "Fale com Stacy sobre Taisia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Message Taisia to meet her with Stacy"
    new "Mande uma mensagem para Taisia para encontrar ela com Stacy"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Veronica"
    new "Fale com Veronica"

    # game/code/data/quests/quests_theater.rpy:3
    old "Progress Kellie's storyline"
    new "Avance a história da Kellie"

    # game/code/data/quests/quests_theater.rpy:3
    old "Wait for Veronica to text you"
    new "Aguarde a mensagem da Veronica"

    # game/code/data/quests/quests_theater.rpy:3
    old "Text Veronica"
    new "Mande uma mensagem para Veronica"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Veronica in the afternoon"
    new "Fale com Veronica de Tarde"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Kellie in the afternoon at Theater"
    new "Fale com Kellie de tarde no Teatro"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk with Taisia about moving in"
    new "Fale com Taisia sobre se mudar"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk with Taisia about moving in after finishing the renovations"
    new "Fale com Taisia sobre se mudar depois de terminar a reforma"

    # game/code/data/quests/quests_theater.rpy:3
    old "Visit Taisia in her room in the evening"
    new "Visite a Taisia no quarto dela no Fim da Tarde"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk with Veronica in the afternoon"
    new "Fale com Veronica de tarde"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Stacy during the day to interview Taisia"
    new "Fale com Stacy durante o dia para entrevistar Taisia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Progress Denise's storyline"
    new "Avance a história de Denise"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Kellie at her dressing room"
    new "Fale com Kellie no camarim dela"

    # game/code/data/quests/quests_theater.rpy:3
    old "Progress Veronica's storyline"
    new "Avance a história de Veronica"

    # game/code/data/quests/quests_theater.rpy:3
    old "Have $50"
    new "Tenha $50"

    # game/code/data/quests/quests_theater.rpy:3
    old "Text Taisia from your phone"
    new "Envie mensagem para Taisia no celular"

    # game/code/data/quests/quests_theater.rpy:3
    old "Finish texting Taisia"
    new "Termine de enviar mensagem para Taisia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Veronica at the stage"
    new "Fale com Veronica no palco"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Veronica on the stage"
    new "Fale com Veronica no palco"

    # game/code/data/quests/quests_theater.rpy:3
    old "Invite Stacy to the amusement park during the day"
    new "Convide Stacy para o parque de diversões durante o dia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Go to the Theater for an audition during the day"
    new "Vá ao teatro para uma entrevista durante o dia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Progress Veronica's and Kellie's storylines"
    new "Avance as histórias de Veronica e Kellie"

    # game/code/data/quests/quests_theater.rpy:3
    old "Do the Saturday show at least 3 times and have 10 total Relationship Points with the Theater girls"
    new "Faça o show de sábado pelo menos 3 vezes e tenha 10 Pontos de Relacionamento no total com as garotas do teatro"

    # game/code/data/quests/quests_theater.rpy:3
    old "Visit the Theater at noon"
    new "Visite o Teatro no começo da tarde"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Denise about the Wurst Delivery sponsorship in her office"
    new "Fale com Denise sobre o patrocínio da Wurst Delivery no escritório dela"

    # game/code/data/quests/quests_theater.rpy:3
    old "Do a Theater show on Saturday"
    new "Faça um show no teatro no sábado"

    # game/code/data/quests/quests_theater.rpy:3
    old "Ask Denise how the play performed"
    new "Pergunte para Denise como a peça foi"

    # game/code/data/quests/quests_theater.rpy:3
    old "Progress the Theater Story"
    new "Avance a história do Teatro"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Denise in her office"
    new "Fale com Denise no escritório dela"

    # game/code/data/quests/quests_theater.rpy:3
    old "Practice with Denise in her office"
    new "Treine com Denise no escritório dela"

    # game/code/data/quests/quests_theater.rpy:3
    old "Wait a day"
    new "Espere um dia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk with Taisia about \"work\" during the daytime"
    new "Fale com Taisia sobre \"trabalho\" durante o dia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Go to the Theater to meet Taisia"
    new "Vá ao teatro para encontrar Taisia"

    # game/code/data/quests/quests_theater.rpy:3
    old "Buy the 'Stars Weekly' magazine from the store"
    new "Compre a revista 'Estrelas Da Semana' na loja"

    # game/code/data/quests/quests_theater.rpy:3
    old "Get your Film and TV topic to 3"
    new "Leve seu tópico de Cinema e TV a nível 3"

    # game/code/data/quests/quests_theater.rpy:3
    old "Do the Saturday show at least [var]/3 times"
    new "Faça o show de sábado pelo menos [var]/3 vezes"

    # game/code/data/quests/quests_theater.rpy:3
    old "Have [var]/10 total Relationship Points with the Theater girls"
    new "Tenha [var]/10 Pontos de Relacionamento no total com as garotas do teatro"

    # game/code/data/quests/quests_theater.rpy:3
    old "Do the Saturday show at least [var0]/3 times and have [var1]/10 total Relationship Points with the Theater girls"
    new "Faça o show de sábado pelo menos [var0]/3 vezes e tenha [var1]/10 Pontos de Relacionamento no total com as garotas do teatro"

    # game/code/data/quests/quests_theater.rpy:3
    old "Have $[var]/$50"
    new "Tenha $[var]/$50"

    # game/code/data/quests/quests_theater.rpy:3
    old "Have $[var]/$100 and invite Taisia to make a film"
    new "Tenha $[var]/$100 e convide Taisia para fazer um filme"

    # game/code/data/quests/quests_theater.rpy:3
    old "Do the rehearsal on [var!t]"
    new "Faça o ensaio em [var!t]"

    # game/code/data/quests/quests_theater.rpy:3
    old "Talk to Nelson during the day about sponsoring the new show"
    new "Fale com Nelson durante o dia sobre patrocinar o novo show"

