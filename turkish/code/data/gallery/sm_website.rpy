translate turkish strings:

    # game/code/data/gallery/sm_website.rpy:1
    old "Red-haired maid fucked hard"
    new "Kızıl saçlı hizmetçi sert sikiliyor"

    # game/code/data/gallery/sm_website.rpy:1
    old "Mr. CEO Man"
    new "Bay CEO Adam"

    # game/code/data/gallery/sm_website.rpy:1
    old "Maid"
    new "Hizmetçi"

    # game/code/data/gallery/sm_website.rpy:1
    old "Sexy brunette fucked in a car"
    new "Seksi kumral arabada sikiliyor"

    # game/code/data/gallery/sm_website.rpy:1
    old "Officer Johnson"
    new "Memur Johnson"

    # game/code/data/gallery/sm_website.rpy:1
    old "Street Racer"
    new "Sokak Yarışçısı"

    # game/code/data/gallery/sm_website.rpy:1
    old "Slutty redhead farmer girl gets her tight ass pounded"
    new "Sürtük kızıl saçlı çiftçi kızının dar kıçı dövülüyor"

    # game/code/data/gallery/sm_website.rpy:1
    old "Farmhand Jon"
    new "Çiftlik İşçisi Jon"

    # game/code/data/gallery/sm_website.rpy:1
    old "Ms. Jones"
    new "Bayan Jones"

    # game/code/data/gallery/sm_website.rpy:1
    old "Curse of the Pirate Queen"
    new "Korsan Kraliçesinin Laneti"

    # game/code/data/gallery/sm_website.rpy:1
    old "Dickhart"
    new "Dickhart"

    # game/code/data/gallery/sm_website.rpy:1
    old "Tempestra"
    new "Tempestra"

    # game/code/data/gallery/sm_website.rpy:1
    old "Searose"
    new "Searose"

    # game/code/data/gallery/sm_website.rpy:1
    old "Star Voyage VI: Babe-lyon 9"
    new "Star Voyage VI: Babe-lyon 9"

    # game/code/data/gallery/sm_website.rpy:1
    old "Captain Ramses Hornstar"
    new "Kaptan Ramses Hornstar"

    # game/code/data/gallery/sm_website.rpy:1
    old "Lt. Luffie Orion"
    new "Yzb. Luffie Orion"

    # game/code/data/gallery/sm_website.rpy:1
    old "Lieutenant Kira Solo"
    new "Yüzbaşı Kira Solo"

    # game/code/data/gallery/sm_website.rpy:1
    old "Doctor Jalerra"
    new "Doktor Jalerra"

    # game/code/data/gallery/sm_website.rpy:1
    old "Spectre"
    new "Spectre"

    # game/code/data/gallery/sm_website.rpy:1
    old "Captain Kimora"
    new "Kaptan Kimora"

