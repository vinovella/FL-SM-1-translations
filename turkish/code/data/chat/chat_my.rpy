translate turkish strings:

    # game/code/data/chat/chat_my.rpy:3
    old "Hey, Mom. I just wanted to say thank you for all your help working on the studio"
    new "Merhaba anne. Stüdyodaki tüm yardımların için teşekkür etmek istedim."

    # game/code/data/chat/chat_my.rpy:3
    old "Hey, Melony. I just wanted to say thank you for all your help working on the studio"
    new "Selam, Melony. Stüdyodaki tüm yardımların için teşekkür etmek istedim."

    # game/code/data/chat/chat_my.rpy:3
    old "Of course, sweetie! Happy to help"
    new "Elbette, tatlım! Yardım etmekten mutluluk duyarım."

    # game/code/data/chat/chat_my.rpy:3
    old "I was wondering if I could maybe treat you to dinner? As a way to show my gratitude?"
    new "Acaba size bir akşam yemeği ısmarlayabilir miyim? Minnettarlığımı göstermenin bir yolu olarak?"

    # game/code/data/chat/chat_my.rpy:3
    old "I would love that!"
    new "Bunu çok isterim!"

    # game/code/data/chat/chat_my.rpy:3
    old "Awesome! I'll send you the address of the restaurant. Is 7 ok?"
    new "Harika! Sana restoranın adresini gönderirim. 7 uygun mu?"

    # game/code/data/chat/chat_my.rpy:3
    old "Sounds great to me!"
    new "Kulağa harika geliyor!"

    # game/code/data/chat/chat_my.rpy:3
    old "See you soon!"
    new "Yakında görüşürüz!"

