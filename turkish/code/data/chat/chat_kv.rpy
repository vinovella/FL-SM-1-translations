translate turkish strings:

    # game/code/data/chat/chat_kv.rpy:3
    old "Hey Kanya."
    new "Merhaba Kanya."

    # game/code/data/chat/chat_kv.rpy:3
    old "Hey stud."
    new "Merhaba Stud."

    # game/code/data/chat/chat_kv.rpy:3
    old "Hiya [mcname!i]."
    new "Merhaba [mcname!i]."

    # game/code/data/chat/chat_kv.rpy:3
    old "What's cooking?"
    new "Ne pişiriyorsun?"

    # game/code/data/chat/chat_kv.rpy:3
    old "Stacy and I are planning to cook up a new film 🎬🎬🎬"
    new "Stacy ve ben yeni bir film hazırlamayı planlıyoruz 🎬🎬🎬"

    # game/code/data/chat/chat_kv.rpy:3
    old "Asking me to be the 🎥 girl again?"
    new "Benden yine 🎥 kızı olmamı mı istiyorsun?"

    # game/code/data/chat/chat_kv.rpy:3
    old "You know me too well."
    new "Beni çok iyi tanıyorsun."

    # game/code/data/chat/chat_kv.rpy:3
    old "We haven't figured out the details."
    new "Detayları henüz çözemedik."

    # game/code/data/chat/chat_kv.rpy:3
    old "But the client wants to focus on anal 🍑"
    new "Ama müşteri anala odaklanmak istiyor 🍑"

    # game/code/data/chat/chat_kv.rpy:3
    old "Stacy might be in trouble, lol."
    new "Stacy'nin başı belada olabilir, lol."

    # game/code/data/chat/chat_kv.rpy:3
    old "She'll be fine."
    new "O iyi olacak."

    # game/code/data/chat/chat_kv.rpy:3
    old "Not our first trip into the mud."
    new "Çamura ilk yolculuğumuz değil."

    # game/code/data/chat/chat_kv.rpy:3
    old "Eww. So gross. Haha. Text me when you need me."
    new "Eww. Çok iğrenç. Haha. Bana ihtiyacın olduğunda mesaj at."

    # game/code/data/chat/chat_kv.rpy:3
    old "Sure!"
    new "Elbette!"

translate turkish strings:

    # game/code/data/chat/chat_kv.rpy:3
    old "What's this one going to be about?"
    new "Bu ne hakkında olacak?"

