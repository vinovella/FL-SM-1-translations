translate deutsch strings:

    # game/code/data/chat/chat_kv.rpy:3
    old "Hey Kanya."
    new "Hey Kanya."

    # game/code/data/chat/chat_kv.rpy:3
    old "Hey stud."
    new "Hey, Hengst."

    # game/code/data/chat/chat_kv.rpy:3
    old "Hiya [mcname!i]."
    new "Wie geht es dir [mcname!i]."

    # game/code/data/chat/chat_kv.rpy:3
    old "What's cooking?"
    new "Was gibts?"

    # game/code/data/chat/chat_kv.rpy:3
    old "Stacy and I are planning to cook up a new film 🎬🎬🎬"
    new "Stacy und ich planen, einen neuen Film zu drehen 🎬🎬🎬"

    # game/code/data/chat/chat_kv.rpy:3
    old "Asking me to be the 🎥 girl again?"
    new "Willst du mich fragen, ob ich wieder das 🎥 Mädchen sein werde?"

    # game/code/data/chat/chat_kv.rpy:3
    old "You know me too well."
    new "Du kennst mich zu gut."

    # game/code/data/chat/chat_kv.rpy:3
    old "We haven't figured out the details."
    new "Wir haben die Details noch nicht geklärt."

    # game/code/data/chat/chat_kv.rpy:3
    old "But the client wants to focus on anal 🍑"
    new "Aber der Kunde möchte sich auf Anal konzentrieren 🍑"

    # game/code/data/chat/chat_kv.rpy:3
    old "Stacy might be in trouble, lol."
    new "Stacy könnte in Schwierigkeiten sein, lol."

    # game/code/data/chat/chat_kv.rpy:3
    old "She'll be fine."
    new "Ihr wird es gut gehen."

    # game/code/data/chat/chat_kv.rpy:3
    old "Not our first trip into the mud."
    new "Nicht unsere erste Reise in den Schlamm."

    # game/code/data/chat/chat_kv.rpy:3
    old "Eww. So gross. Haha. Text me when you need me."
    new "Igitt. Wie eklig. Haha. Schreib mir, wenn du mich brauchst."

    # game/code/data/chat/chat_kv.rpy:3
    old "Sure!"
    new "Klar!"

translate deutsch strings:

    # game/code/data/chat/chat_kv.rpy:3
    old "What's this one going to be about?"
    new "Worum wird es gehen?"

