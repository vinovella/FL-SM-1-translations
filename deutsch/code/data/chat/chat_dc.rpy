translate deutsch strings:

    # game/code/data/chat/chat_dc.rpy:3
    old "Hey, [mcname!i]! I just wanted to say that I'm really happy I finally told you everything, and I am really excited for that future date! 😜"
    new "Hey, [mcname!i]! Ich wollte nur sagen, dass ich wirklich froh bin, dir endlich alles erzählt zu haben, und ich freue mich schon sehr auf unser zukünftiges Date! 😜"

    # game/code/data/chat/chat_dc.rpy:3
    old "In fact, what are you doing tonight?"
    new "Was machst du eigentlich heute Abend?"

    # game/code/data/chat/chat_dc.rpy:3
    old "I'm actually stuck working the next few night shifts 😞"
    new "Ich muss tatsächlich die nächsten paar Nächte arbeiten. 😞"

    # game/code/data/chat/chat_dc.rpy:3
    old "Booo! Well, maybe I'll run into you in the park"
    new "Buh! Na ja, vielleicht treffe ich dich ja im Park."

    # game/code/data/chat/chat_dc.rpy:3
    old "I would love that 😊"
    new "Das würde mir sehr gefallen. 😊"

    # game/code/data/chat/chat_dc.rpy:41
    old "Hey Debbie! 👋"
    new "Hey Debbie! 👋"

    # game/code/data/chat/chat_dc.rpy:41
    old "Hey, [mcname!i]! What can I do for you?"
    new "Hey, [mcname!i]! Was kann ich für dich tun?"

    # game/code/data/chat/chat_dc.rpy:41
    old "Well I promised you a date and I was wondering if you'd want to go on a date soon? 😊"
    new "Nun, ich habe dir ein Date versprochen und habe mich gefragt, ob du bald mit mir ausgehen möchtest? 😊"

    # game/code/data/chat/chat_dc.rpy:41
    old "That sounds delightful!"
    new "Das klingt wunderbar!"

    # game/code/data/chat/chat_dc.rpy:41
    old "There's this nice restaurant... wanna go there?"
    new "Es gibt da dieses nette Restaurant... möchtest du hingehen?"

    # game/code/data/chat/chat_dc.rpy:41
    old "It sounds like a date, [mcname!i] 😉"
    new "Das klingt nach einem Date, [mcname!i] 😉"

translate deutsch strings:

    # game/code/data/chat/chat_dc.rpy:3
    old "Hey Debbie! I'm glad we talked too. I'm also pretty pumped for this date"
    new "Hey Debbie! Ich bin auch froh, dass wir miteinander gesprochen haben. Ich bin auch ziemlich aufgeregt wegen diesem Date"

    # game/code/data/chat/chat_dc.rpy:41
    old "I'll send you the address! See you in a bit 😏"
    new "Ich schicke dir die Adresse! Bis gleich 😏"

