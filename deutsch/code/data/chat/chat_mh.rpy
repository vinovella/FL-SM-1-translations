translate deutsch strings:

    # game/code/data/chat/chat_mh.rpy:3
    old "Hey! What's up?"
    new "Hey! Was geht?"

    # game/code/data/chat/chat_mh.rpy:3
    old "Not much, thinking about you 😎"
    new "Nicht viel, ich denke an dich. 😎"

    # game/code/data/chat/chat_mh.rpy:3
    old "Same here. I'm at home, you want to come over?"
    new "Ich auch. Ich bin zu Hause, möchtest du vorbeikommen?"

    # game/code/data/chat/chat_mh.rpy:3
    old "ON MY WAY!"
    new "ICH BIN UNTERWEGS!"

    # game/code/data/chat/chat_mh.rpy:37
    old "Hey, [mcname!i]. I have the next few mornings off. I was wondering if I could stop by and see the new renovations?"
    new "Hey, [mcname!i]. Ich habe die nächsten paar Vormittage frei. Ich habe mich gefragt, ob ich vorbeikommen und mir die neuen Renovierungen ansehen könnte?"

    # game/code/data/chat/chat_mh.rpy:37
    old "Yeah! I'm totally free this morning, stop on by!"
    new "Ja! Ich habe heute Vormittag Zeit, komm doch vorbei!"

    # game/code/data/chat/chat_mh.rpy:37
    old "I'll be over in a jif"
    new "Ich bin gleich da."

translate deutsch strings:

    # game/code/data/chat/chat_mh.rpy:37
    old "I'll be over in a jiff"
    new "I'll be over in a jiff"

