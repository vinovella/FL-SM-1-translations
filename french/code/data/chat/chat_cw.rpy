translate french strings:

    # game/code/data/chat/chat_cw.rpy:3
    old "Hello, Mr. Young."
    new "Bonjour, M. Young."

    # game/code/data/chat/chat_cw.rpy:3
    old "Hi, Ms. Claire."
    new "Bonjour, Mme Claire."

    # game/code/data/chat/chat_cw.rpy:3
    old "Something has come up. Please meet me Friday evening at the office."
    new "Un imprévu est survenu. Veuillez me retrouver vendredi soir au bureau."

    # game/code/data/chat/chat_cw.rpy:3
    old "I may need your help with something."
    new "J’aurai peut-être besoin de votre aide pour quelque chose."

    # game/code/data/chat/chat_cw.rpy:3
    old "Okay."
    new "D’accord."

    # game/code/data/chat/chat_cw.rpy:3
    old "Goodbye."
    new "Au revoir."

    # game/code/data/chat/chat_cw.rpy:3
    old "Hi, Ms. Watts."
    new "Bonjour, Mme Watts."

