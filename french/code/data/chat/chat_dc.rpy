translate french strings:

    # game/code/data/chat/chat_dc.rpy:3
    old "Hey, [mcname!i]! I just wanted to say that I'm really happy I finally told you everything, and I am really excited for that future date! 😜"
    new "Hé, [mcname!i] ! Je voulais juste dire que je suis super contente de t’avoir enfin tout dit, et je suis trop excitée pour notre futur rendez-vous ! 😜"

    # game/code/data/chat/chat_dc.rpy:3
    old "Hey Debbie! Im glad we talked too. Im also pretty pumped for this date"
    new "Hey Debbie ! Je suis content qu’on ait parlé aussi. Moi aussi je suis super motivé pour ce rendez-vous"

    # game/code/data/chat/chat_dc.rpy:3
    old "In fact, what are you doing tonight?"
    new "D’ailleurs, tu fais quoi ce soir ?"

    # game/code/data/chat/chat_dc.rpy:3
    old "I'm actually stuck working the next few night shifts 😞"
    new "En fait je suis coincée avec quelques gardes de nuit 😞"

    # game/code/data/chat/chat_dc.rpy:3
    old "Booo! Well, maybe I'll run into you in the park"
    new "Bouh ! Bon, peut-être que je te croiserai au parc"

    # game/code/data/chat/chat_dc.rpy:3
    old "I would love that 😊"
    new "J’adorerais 😊"

    # game/code/data/chat/chat_dc.rpy:41
    old "Hey Debbie! 👋"
    new "Hey Debbie ! 👋"

    # game/code/data/chat/chat_dc.rpy:41
    old "Hey, [mcname!i]! What can I do for you?"
    new "Hé, [mcname!i] ! Qu’est-ce que je peux faire pour toi ?"

    # game/code/data/chat/chat_dc.rpy:41
    old "Well I promised you a date and I was wondering if you'd want to go on a date soon? 😊"
    new "Eh bien, je t’ai promis un rendez-vous et je me demandais si tu voudrais qu’on sorte bientôt ? 😊"

    # game/code/data/chat/chat_dc.rpy:41
    old "That sounds delightful!"
    new "Ça me ferait super plaisir !"

    # game/code/data/chat/chat_dc.rpy:41
    old "There's this nice restaurant... wanna go there?"
    new "Il y a un resto sympa… tu veux qu’on y aille ?"

    # game/code/data/chat/chat_dc.rpy:41
    old "It sounds like a date, [mcname!i] 😉"
    new "Ça ressemble bien à un rendez-vous, [mcname!i] 😉"

    # game/code/data/chat/chat_dc.rpy:41
    old "I'll send  you the address! See you in a bit 😏"
    new "Je t’envoie l’adresse ! À tout à l’heure 😏"

    # game/code/data/chat/chat_dc.rpy:3
    old "Hey Debbie! I'm glad we talked too. I'm also pretty pumped for this date"
    new "Salut Debbie ! Je suis content qu’on ait parlé aussi. Je suis vraiment impatient pour ce rendez-vous."

    # game/code/data/chat/chat_dc.rpy:41
    old "I'll send you the address! See you in a bit 😏"
    new "Je t’envoie l’adresse ! À tout à l’heure 😏"

#Translate by BlackViper_
