# game/code/scenes/theatre/tl/sm1cs-tl001i.rpy:6
translate french sm1cs_tl001i_c7a0fb24:

    # mc "Hey, Taisia. I wanted to talk to you-"
    mc "Hey, Taisia. Je voulais te parler—"

# game/code/scenes/theatre/tl/sm1cs-tl001i.rpy:8
translate french sm1cs_tl001i_6f85c701:

    # tl "Are you telling me I'm a part of the team?"
    tl "Tu es en train de me dire que je fais partie de l’équipe ?"

# game/code/scenes/theatre/tl/sm1cs-tl001i.rpy:10
translate french sm1cs_tl001i_a480849b:

    # mc "Yeah! That you are."
    mc "Ouais ! Exactement."

# game/code/scenes/theatre/tl/sm1cs-tl001i.rpy:12
translate french sm1cs_tl001i_c2678b96:

    # tl "Hell yeah! So when are we filming our first porn?"
    tl "Putain ouais ! Alors, c’est quand qu’on tourne notre premier porno ?"

# game/code/scenes/theatre/tl/sm1cs-tl001i.rpy:16
translate french sm1cs_tl001i_e6f0485a:

    # mc "Woah, uh, maybe we shouldn't talk about this {i}here{/i}."
    mc "Woah, euh... on devrait peut-être éviter d’en parler {i}ici{/i}."

# game/code/scenes/theatre/tl/sm1cs-tl001i.rpy:18
translate french sm1cs_tl001i_8f19cb01:

    # tl "Fucking whatever."
    tl "Pfff, comme tu veux."

# game/code/scenes/theatre/tl/sm1cs-tl001i.rpy:19
translate french sm1cs_tl001i_725c9a08:

    # mc "But if you have some time, we can go to the studio and talk some more about it?"
    mc "Mais si t’as un peu de temps, on peut aller au studio en discuter un peu plus ?"

# game/code/scenes/theatre/tl/sm1cs-tl001i.rpy:20
translate french sm1cs_tl001i_b87895d0:

    # tl "Lead the way, leading man."
    tl "Montre le chemin, monsieur la star."

#Translate by BlackViper_

