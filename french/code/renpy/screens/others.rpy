translate french strings:

    # game/code/renpy/screens/others.rpy:27
    old "Sync your saves using Ren'Py Sync server"
    new "Synchronise tes sauvegardes avec le serveur de synchro Ren'Py"

    # game/code/renpy/screens/others.rpy:29
    old "Upload Saves"
    new "Envoyer les sauvegardes"

    # game/code/renpy/screens/others.rpy:30
    old "Download Saves"
    new "Télécharger les sauvegardes"

    # game/code/renpy/screens/others.rpy:75
    old "SKIP"
    new "PASSER"

    # game/code/renpy/screens/others.rpy:222
    old "Please enter your name here!"
    new "Entre ton nom ici !"

    # game/code/renpy/screens/others.rpy:230
    old "{u}DONE{/u}"
    new "{u}TERMINÉ{/u}"

    # game/code/renpy/screens/others.rpy:277
    old "How do you want to name your save?"
    new "Comment veux-tu nommer ta sauvegarde ?"

    # game/code/renpy/screens/others.rpy:13
    old "Sync your saves using Ren'Py Sync Server"
    new "Synchronisez vos sauvegardes avec le serveur de synchronisation Ren'Py"

    # game/code/renpy/screens/others.rpy:139
    old "Please enter your Local IP number:"
    new "Veuillez entrer votre adresse IP locale :"

    # game/code/renpy/screens/others.rpy:142
    old "Please enter your HTTP Port number:"
    new "Veuillez entrer votre numéro de port HTTP :"

    # game/code/renpy/screens/others.rpy:34
    old "We collect only anonymized data to better understand which paths, characters, scenes, and fetishes are most popular and to identify areas for improvement."
    new "Nous ne collectons que des données anonymisées afin de mieux comprendre quels chemins, personnages, scènes et fétiches sont les plus populaires et d'identifier les domaines à améliorer."

    # game/code/renpy/screens/others.rpy:35
    old "NO PERSONAL INFORMATION IS COLLECTED."
    new "AUCUNE INFORMATION PERSONNELLE N'EST COLLECTÉE."

    # game/code/renpy/screens/others.rpy:36
    old "Would you like to enable this data collection?"
    new "Souhaitez-vous activer cette collecte de données ?"

    # game/code/renpy/screens/others.rpy:173
    old "How do you want to name your Season 2 save?"
    new "Comment voulez-vous nommer votre sauvegarde de la Saison 2 ?"

    # game/code/renpy/screens/others.rpy:12
    old "If the recent minigames/puzzles feel a bit tricky, that's totally okay."
    new "Si les récents mini-jeux/casse-têtes semblent un peu difficiles, c'est tout à fait normal."

    # game/code/renpy/screens/others.rpy:13
    old "You can disable minigames/puzzles for this playthrough and continue the story without them."
    new "Vous pouvez désactiver les mini-jeux/casse-têtes pour cette partie et continuer l'histoire sans eux."

    # game/code/renpy/screens/others.rpy:14
    old "Once disabled, minigames can't be turned back on for this playthrough, and minigame achievements won't be earned."
    new "Une fois désactivés, les mini-jeux ne peuvent pas être réactivés pour cette partie, et les succès des mini-jeux ne seront pas obtenus."

    # game/code/renpy/screens/others.rpy:16
    old "Disable Minigames"
    new "Désactiver les mini-jeux"

    # game/code/renpy/screens/others.rpy:20
    old "Keep Minigames On"
    new "Garder les mini-jeux activés"

