translate spanish strings:

    # game/code/data/chat/chat_vs.rpy:3
    old "Hey, can we talk?"
    new "Oye, ¿podemos hablar?"

    # game/code/data/chat/chat_vs.rpy:3
    old "Sure. Everything good?"
    new "Claro. ¿Todo bien?"

    # game/code/data/chat/chat_vs.rpy:3
    old "Totally!"
    new "¡Totalmente!"

    # game/code/data/chat/chat_vs.rpy:3
    old "Well, not exactly. Kellie is acting weird around me."
    new "Bueno, no exactamente. Kellie está actuando rara conmigo."

    # game/code/data/chat/chat_vs.rpy:3
    old "I don't get it."
    new "No lo entiendo."

    # game/code/data/chat/chat_vs.rpy:3
    old "Well, we did catch her rubbing her clam."
    new "Bueno, la atrapamos frotando su almeja."

    # game/code/data/chat/chat_vs.rpy:3
    old "Hahah. Rubbing her clam? You're ridiculous."
    new "Jajaja. ¿Frotando su almeja? Eres ridículo."

    # game/code/data/chat/chat_vs.rpy:3
    old "I tried to talk to her after, but she's avoiding me."
    new "Traté de hablar con ella después, pero ella me está evitando."

    # game/code/data/chat/chat_vs.rpy:3
    old "I don't get her."
    new "No la entiendo."

    # game/code/data/chat/chat_vs.rpy:3
    old "I think I know what's up with her."
    new "Creo que sé lo que pasa con ella."

    # game/code/data/chat/chat_vs.rpy:3
    old "But I can't say why."
    new "Pero no puedo decir por qué."

    # game/code/data/chat/chat_vs.rpy:3
    old "What? You can't tease me like that, [mcname!i]."
    new "¿Qué? No puedes burlarte de mí de esa manera, [mcname!i]."

    # game/code/data/chat/chat_vs.rpy:3
    old "Wait, unless this is a FL thing."
    new "Espera, a menos que esto sea una cosa del Localizador de Fetiches."

    # game/code/data/chat/chat_vs.rpy:3
    old "Then tease away."
    new "Entonces, bromea."

    # game/code/data/chat/chat_vs.rpy:3
    old "No. It's not about that. Kellie didn't want me to tell you."
    new "No. No se trata de eso. Kellie no quería que te lo dijera."

    # game/code/data/chat/chat_vs.rpy:3
    old "LOL. Well, now you have to tell me!"
    new "JAJAJA. Bueno, ¡ahora tienes que decirme!"

    # game/code/data/chat/chat_vs.rpy:3
    old "Okay. Can you meet me?"
    new "De acuerdo. ¿Puedes reunirte conmigo?"

    # game/code/data/chat/chat_vs.rpy:3
    old "Sure. Find me at the park."
    new "Seguro. Encuéntrame en el parque."

    # game/code/data/chat/chat_vs.rpy:3
    old "Sure."
    new "Está bien."

