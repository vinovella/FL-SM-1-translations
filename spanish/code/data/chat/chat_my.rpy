translate spanish strings:

    # game/code/data/chat/chat_my.rpy:3
    old "Hey, Mom. I just wanted to say thank you for all your help working on the studio"
    new "Hola, mamá. Sólo quería darte las gracias por toda tu ayuda trabajando en el estudio."

    # game/code/data/chat/chat_my.rpy:3
    old "Hey, Melony. I just wanted to say thank you for all your help working on the studio"
    new "Hola, Melony. Sólo quería darte las gracias por toda tu ayuda trabajando en el estudio."

    # game/code/data/chat/chat_my.rpy:3
    old "Of course, sweetie! Happy to help"
    new "¡Por supuesto, cariño! Encantada de ayudar."

    # game/code/data/chat/chat_my.rpy:3
    old "I was wondering if I could maybe treat you to dinner? As a way to show my gratitude?"
    new "¿Me preguntaba si podría invitarte a cenar? ¿Como una forma de mostrar mi gratitud?"

    # game/code/data/chat/chat_my.rpy:3
    old "I would love that!"
    new "¡Me encantaría eso!"

    # game/code/data/chat/chat_my.rpy:3
    old "Awesome! I'll send you the address of the restaurant. Is 7 ok?"
    new "¡Impresionante! Te enviaré la dirección del restaurante. ¿A las 7 está bien?"

    # game/code/data/chat/chat_my.rpy:3
    old "Sounds great to me!"
    new "¡Suena genial para mí!"

    # game/code/data/chat/chat_my.rpy:3
    old "See you soon!"
    new "¡Nos vemos pronto!"

