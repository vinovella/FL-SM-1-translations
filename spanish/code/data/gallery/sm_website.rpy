translate spanish strings:

    # game/code/data/gallery/sm_website.rpy:1
    old "Red-haired maid fucked hard"
    new "Sirvienta pelirroja follada con fuerza"

    # game/code/data/gallery/sm_website.rpy:1
    old "Mr. CEO Man"
    new "Sr. CEO Man"

    # game/code/data/gallery/sm_website.rpy:1
    old "Maid"
    new "La sirvienta"

    # game/code/data/gallery/sm_website.rpy:1
    old "Sexy brunette fucked in a car"
    new "Morena sexy follada en un auto"

    # game/code/data/gallery/sm_website.rpy:1
    old "Officer Johnson"
    new "Oficial Johnson"

    # game/code/data/gallery/sm_website.rpy:1
    old "Street Racer"
    new "Corredora callejera"

    # game/code/data/gallery/sm_website.rpy:1
    old "Slutty redhead farmer girl gets her tight ass pounded"
    new "Granjera pelirroja y atrevida recibe duro por el trasero"

    # game/code/data/gallery/sm_website.rpy:1
    old "Farmhand Jon"
    new "Jon, el peón de granja"

    # game/code/data/gallery/sm_website.rpy:1
    old "Ms. Jones"
    new "Sra. Jones"

    # game/code/data/gallery/sm_website.rpy:1
    old "Curse of the Pirate Queen"
    new "La maldición de la Reina Pirata"

    # game/code/data/gallery/sm_website.rpy:1
    old "Dickhart"
    new "Dickhart"

    # game/code/data/gallery/sm_website.rpy:1
    old "Tempestra"
    new "Tempestra"

    # game/code/data/gallery/sm_website.rpy:1
    old "Searose"
    new "Searose"

    # game/code/data/gallery/sm_website.rpy:1
    old "Star Voyage VI: Babe-lyon 9"
    new "Star Voyage VI: Babe-lyon 9"

    # game/code/data/gallery/sm_website.rpy:1
    old "Captain Ramses Hornstar"
    new "Capitán Ramses Hornstar"

    # game/code/data/gallery/sm_website.rpy:1
    old "Lt. Luffie Orion"
    new "Tte. Luffie Orion"

    # game/code/data/gallery/sm_website.rpy:1
    old "Lieutenant Kira Solo"
    new "Teniente Kira Solo"

    # game/code/data/gallery/sm_website.rpy:1
    old "Doctor Jalerra"
    new "Doctora Jalerra"

    # game/code/data/gallery/sm_website.rpy:1
    old "Spectre"
    new "Spectre"

    # game/code/data/gallery/sm_website.rpy:1
    old "Captain Kimora"
    new "Capitana Kimora"

