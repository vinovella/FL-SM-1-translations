# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:6
translate spanish sm1mv02s08i_3e248f0f:
    # mc "Hey, Stacy."
    mc "Hola, Stacy."

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:8
translate spanish sm1mv02s08i_f6d1280e:
    # sy "Hey, handsome."
    sy "Hola, guapo."

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:10
translate spanish sm1mv02s08i_419a6a4a:
    # mc "So are we all set for the big scenes?"
    mc "¿Así que estamos listos para las grandes escenas?"

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:13
translate spanish sm1mv02s08i_308d62a7:
    # mc "I'm eager to put this one to rest."
    mc "Estoy ansioso por dar por terminada esta."

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:15
translate spanish sm1mv02s08i_06898806:
    # sy "Well, I would say we are halfway there."
    sy "Bueno, yo diría que estamos a mitad de camino."

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:19
translate spanish sm1mv02s08i_2b5d1c01:
    # mc "Only halfway? What's the hold up?"
    mc "¿Solo a mitad de camino? ¿Cuál es el retraso?"

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:21
translate spanish sm1mv02s08i_375f0da9:
    # sy "These last two scenes require a lot of CGI graphics to fill in the gaps, [mcname]."
    sy "Estas dos últimas escenas requieren muchos gráficos CGI para rellenar los huecos, [mcname]."

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:22
translate spanish sm1mv02s08i_1b327b2c:
    # sy "So that means we need a lot of cheddar."
    sy "Así que eso significa que necesitamos mucho *cheddar*."

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:26
translate spanish sm1mv02s08i_33d7eda6:
    # mc "What does cheese have to do with CGI?"
    mc "¿Qué tiene que ver el queso con el CGI?"

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:28
translate spanish sm1mv02s08i_60e11c93:
    # sy "Hahaha."
    sy "Jajaja."

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:29
translate spanish sm1mv02s08i_7724524f:
    # sy "Cash, man. Cash!"
    sy "¡Dinero, hombre. Dinero!"

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:32
translate spanish sm1mv02s08i_121c5b7c:
    # sy "These last two scenes are the real deal, and the movie bank is tapped out, so we need a cash infusion."
    sy "Estas dos últimas escenas son de verdad, y el banco de la película está agotado, así que necesitamos una inyección de efectivo."

# game/code/scenes/movies/movie02/sm1mv02s08i.rpy:34
translate spanish sm1mv02s08i_f088434a:
    # mc "Alright. I'll see what I can do."
    mc "De acuerdo. Veré qué puedo hacer."
