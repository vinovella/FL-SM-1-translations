# game/code/scenes/movies/movie01/sm1mv01s06i.rpy:6
translate spanish sm1mv01s06i_071ddc41:
    # mc "Hi Taisia."
    mc "Hola, Taisia."

# game/code/scenes/movies/movie01/sm1mv01s06i.rpy:10
translate spanish sm1mv01s06i_021b5030:
    # tl "Hey handsome."
    tl "Hola, guapo."

# game/code/scenes/movies/movie01/sm1mv01s06i.rpy:12
translate spanish sm1mv01s06i_da1676be:
    # mc "Ready to film the second scene?"
    mc "¿Lista para filmar la segunda escena?"

# game/code/scenes/movies/movie01/sm1mv01s06i.rpy:16
translate spanish sm1mv01s06i_490f2fb4:
    # tl "I was born ready!"
    tl "¡Nací lista!"

# game/code/scenes/movies/movie01/sm1mv01s06i.rpy:18
translate spanish sm1mv01s06i_87bf65c4:
    # mc "Haha. Time to film some porn!"
    mc "Jaja. ¡Es hora de filmar algo de porno!"
