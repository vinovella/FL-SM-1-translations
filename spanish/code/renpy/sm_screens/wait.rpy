# TODO: Translation updated at 2024-05-05 11:07

translate spanish strings:

    # game/code/renpy/sm_screens/wait.rpy:6
    old "Delivering Wurst..."
    new "Delivering Wurst..."

    # game/code/renpy/sm_screens/wait.rpy:8
    old "Coding Solutions..."
    new "Coding Solutions..."

    # game/code/renpy/sm_screens/wait.rpy:10
    old "Reading Book..."
    new "Reading Book..."

    # game/code/renpy/sm_screens/wait.rpy:12
    old "Working in the Theater..."
    new "Working in the Theater..."

